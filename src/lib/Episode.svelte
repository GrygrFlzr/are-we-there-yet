<script lang="ts">
	import InformationCircleSmallSvg from '$lib/Icon/InformationCircleSmall.svg?raw';
	import Position from '$lib/Position.svelte';
	import { allMarked, deduplicateStaff, hasAired, someMarked } from '$lib/utils';
	...
	$: uniqueStaff = deduplicateStaff(episode.staff.filter((staff) => !staff.finished));
		...
		{#if latest}
			<div class="absolute bottom-2 left-3 text-xs text-gray-400 dark:text-gray-300">
				<div class="flex flex-row gap-1 text-left">
					{#if status}
						<!-- Status Tooltip -->
						<span class="z-10 inline-block h-4 w-4 relative text-blue-400 group">
							{@html InformationCircleSmallSvg}
							<span
